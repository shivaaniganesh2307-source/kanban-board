...
const supabaseAnonKey =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNnd3V0dnBtb2hoeWhob3dld3BuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUwNDgyNzUsImV4cCI6MjA5MDYyNDI3NX0.KsMf9u-VRX9_b4DaAgbXsc8eBam1sEG3DP0Zn-DODRQ";
...
function App() {
  ...
  const onDragEnd = async (result) => {
    ...
    const task = sourceCol.tasks.find((t) => t.id.toString() === draggableId);
    if (!task) return;
  ...
  const createTask = async (task) => {
    ...
    const newTask = { ...task, user_id: user.id, status: task.status || "todo" };
    ...
    if (task.id) {
      ...
      if (user.id !== "guest-1") {
        await supabase
          .from("tasks")
          .insert([{ ...newTask, due_date: task.due_date || null, assignee_id: task.assignee_id || null }]);
      } else {
